<template>
  <div v-if="loading" class="loading">
    ...
    <div class="spinner-placeholder">Loading...</div>
  </div>
  ...
  <div v-else-if="product" class="product-view">
    ...
    <div class="product-page">
      <ProductImageSlider :images="product.media.images" aspect-ratio="1/1" />

      <div class="product-details">
        <h1 class="product-title">{{ product.title }}</h1> 
        <div class="price-container">
          <span class="current-price">${{ product.price.amount }}</span> - <span class="badge text-bg-primary">{{ product.seller.name }}</span>
        ...
        <!-- Add to cart button, size selector, etc -->

        

        <div class="my-3 p-3 bg-body rounded">
          <pre class="pb-2 mb-0">Sheygani waa indheynaya : <span class="badge text-bg-light">{{  favoritesCount }}</span> Qofod</pre>
...
const product : any = ref<Product | null>(null);
...
useHead({
  ...
  meta: [
    { 
      name: 'description', 
      content: computed(() => product.value?.description?.substring(0, 160) || 'Product details page') 
    },
    // Open Graph/Facebook
    { property: 'og:type', content: 'product' },
    { property: 'og:title', content: computed(() => product.value?.title) },
    { property: 'og:description', content: computed(() => product.value?.description?.substring(0, 160)) },
    { property: 'og:image', content: computed(() => product.value?.media?.images?.[0]) },
    
    // Twitter Card
    { name: 'twitter:card', content: 'summary_large_image' },
    { name: 'twitter:title', content: computed(() => `${product.value?.title} - ${product.value?.seller.name}`) },
    { name: 'twitter:description', content: computed(() => product.value?.description?.substring(0, 160)) },
    { name: 'twitter:image', content: computed(() => product.value?.media?.images?.[0]) }
  ]
});
...
async function fetchProductDetails() {
  try {
    const response: any = await client.get(`product/${route.params.id}`);
